...
### P3. 偵錯優先指令預覽
執行複雜更新前，建議用 `MongoRepository<T>.FlattenBsonDocument(rawDoc)` 預覽 `$set` 攤平後的點符號路徑，確認 nested model 對應的欄位名與 Filter 條件符合預期。範例見 `docs/SDK_QuickStart.md`「預覽指令」section。

> **變更歷史**：原條文引用 `UpdateInit` method（已於 2026-05-13 棄用）。
> 棄用理由：`UpdateInit` 內部序列化 logic 與 BsonDocument 存在型別衝突（InvalidCastException），為保持底層穩定，改採教學範例提供之靜態工具進行預覽。
