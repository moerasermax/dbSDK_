...
### 層級劃分：
* **Core Layer (核心層)**: 定義系統合約 (Interfaces) 與業務模型 (Models/Entities)。嚴禁依賴任何外部 SDK。
* **Application Layer (SDK Engine)**: 負責協調業務流程。原 `dbSDKEngine` 應位於此層級，作為 SDK 的主要入口點。
* **Infrastructure Layer (基礎設施層)**: 負責技術實作。包含資料庫驅動 (Drivers) 與倉儲實作 (Repositories)。
...
```mermaid
graph TD
    Entry(Sample / Entry Point) -->|DI| App[Core.Services / Application]
    Entry -->|Reg| Infra[Infrastructure]
    App -->|Define| Core[Core.Interfaces / Models]
