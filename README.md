...
🤝 開發規範 (Development Standards)為了維持系統的純淨度，貢獻代碼時請遵循以下規範：Core 獨立性：核心層項目不得引用任何第三方 NuGet 套件（如 MongoDB.Driver 或 StackExchange.Redis）。依賴方向：依賴箭頭必須永遠指向核心 (Core)，基礎設施層不得反向干涉業務規則。介面先行：所有功能擴充必須先定義 Interface 合約，再進行實作開發。防腐層實踐：任何外部 SDK 的型別（如 BsonDocument）嚴禁滲透進 Core 層，必須在 Infrastructure 層進行 Map 轉換。
