    ...
    public class MongoRepository<T> : IRepository<T>
    {
        ...
        async Task<IResult> IRepository<T>.getData(string ConditionData)
        {
            ...
            try
            {
                using (var cursor = await _Collection.FindAsync(filter))
                {
                    ...
                    this.Result.setResult(true, "[MongoDBSDK]查詢資料成功。", data.ToJson());
                }
        ...
        async Task<IResult> IRepository<T>.insertData(T Data)
        {
            try
            {
                ...
                this.Result.setResult(true, "[MongoDBSDK]資料新增成功。");
            }
            catch (Exception ex)
            {
                this.Result.setErrorResult(MethodBase.GetCurrentMethod()?.Name, ex.Message);
        ...
        async Task<IResult> IRepository<T>.removeData(string ConditionData)
        {
            try
            {
                ...
                this.Result.setResult(true, "[MongoDBSDK]刪除資料成功。");
            }
            catch (Exception ex)
            {
                this.Result.setErrorResult(MethodBase.GetCurrentMethod()?.Name, ex.Message);
        ...
        async Task<IResult> IRepository<T>.updateData(string ConditionData, T UpdateData)
        {
            try
            {
                ...
                this.Result.setResult(true, "[MongoDBSDK]資料更新成功。", queryResult.ToJson());
            }
            catch (Exception ex)
            {
                this.Result.setErrorResult(MethodBase.GetCurrentMethod()?.Name, ex.Message);
